...
namespace Taki_Client
{
    class GameLobbyAdmin : Panel
    {
        ...
        public void Initialize()
        {
            ...
            Bitmap image = new Bitmap("C:\\Users\\yuval\\Desktop\\Taki\\Taki_Client\\Sources\\Taki Logo.jpg");
        ...
        void startGame_Click(object Sender, EventArgs e)
        {
            //    string[]players;
            //    Deck deck = new Deck(new List<Card>());
            //    string playersList, cards;
            //    List<Enemy> enemies = new List<Enemy>();
            //    this.waiting = false;
            //    bool has_started = false;
            Communication.SendMsg(this.sock, "start_game", new string[] { this.jwt });
            GamePanel panel = new GamePanel(this.gameID, this.password, this.jwt, this.sock, new List<Enemy>(), new List<Card>(), new Card("number_card", "green", "1")); 
            ...
            //    while (!has_started)
            //    {
            //        foreach (string msg in responses)
            //        {
            ...
            //            {
            //                dynamic args = JsonConvert.DeserializeObject(json.args.ToString());
            //                playersList = args.players;
            //                cards = args.cards;
            //                playersList = playersList.Substring(1, playersList.Length - 2);
            //                players = playersList.Split(new char[] { ' ', ',' });
            //                int index = 1;
            //                foreach (string player in players)
            //                {
            //                    if (player != "")
            ...
            //                    }
            //                }
            //                List<string> deckCards = JsonConvert.DeserializeObject<List<string>>(cards);
            //                foreach (string card in deckCards)
            //                {
            //                    dynamic jsonCard = JsonConvert.DeserializeObject(card);
            //                    deck.AddCard(new Card(jsonCard.type, jsonCard.color, (int)jsonCard.value));
            //                }
            //                has_started = true;
            //            }
            ...
            //        }
            //        if (!has_started)
            //            responses = Communication.GameHandler(this.sock, "NOY MY TURN", new string[] { });
            //    }
        }
    }
}